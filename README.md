...
转换后的文件输出到 `integrations/<tool>/`（目录已纳入 `.gitignore`，新克隆仓库里为空，请先跑一次 `python scripts/convert.py --tool all` 生成）。`integrations/` 只用于查看导出结果，不作为人工编辑面；目录边界见 [integrations/README.md](integrations/README.md)。`scripts/convert.py` 现在同时提供：
...
├── commands/                  # 斜杠命令（25 个：16 派发器 + 6 本地 runtime + 3 混合低摩擦 CLI）
│   └── opc/
│       ├── opc.md             # /opc 统一自然语言入口（派发 workflow-modes）
│       ├── start.md           # /opc-start 初始化项目（派发 workflow-modes + project-init）
...
│       ├── health.md          # /opc-health 质量体检（本地 runtime：诊断 + safe repair）
│       ├── dashboard.md       # /opc-dashboard 项目仪表盘（纯只读 CLI）
│       ├── stats.md           # /opc-stats 项目指标（纯只读 CLI）
│       ├── intel.md           # /opc-intel 代码库情报（本地 runtime，refresh 子命令走 agent）
│       ├── profile.md         # /opc-profile 开发者画像（本地 runtime，本地 profile 读写）
│       ├── research.md        # /opc-research 研究 runtime（methods / insights / feed / run）
...
│   └── archive/               # 历史设计规格 / 重构提案 / 融合档案
├── marketing/                 # 发布素材
│   ├── README.md              # 当前 vs 历史发布资产边界
│   └── archive/               # 按版本归档的历史 launch 文案
├── website/                   # 官网落地页
│   ├── index.html             # 当前公开网站文案
│   └── README.md              # 部署说明
├── CLAUDE.md                  # AI 系统指令
...
>
> **快速看目录：** 如果你想先理解仓库结构、哪些目录是源码、哪些是生成物/本地运行目录，直接看 **[docs/DIRECTORY-MAP.md](docs/DIRECTORY-MAP.md)**。
>
> **快速看历史：** 如果你在找旧版设计提案或旧发布文案，去 **[docs/archive/README.md](docs/archive/README.md)** 和 **[marketing/README.md](marketing/README.md)**，不要把这些历史文件当成当前使用说明。
...
### 5. 运行市场研究
```
/opc-research run --query "AI 写作市场"
```
抓 feed → 生成 insights → 写入 `.opc/research/` 研究报告。需要把研究结论转成业务取舍时，再继续走 `/opc-business`。
...
> （`/opc-xxx`），它们会派发对应的 dispatcher skill。`/opc-health` `/opc-dashboard` `/opc-stats`
> `/opc-intel` `/opc-profile` `/opc-research` 这 6 个入口允许直接调本地 runtime 作为等价替代：
> 其中 `/opc-dashboard` `/opc-stats` 是纯只读，`/opc-intel refresh` 仍走 dispatcher，`/opc-health --repair`
> 与 `/opc-research run` 会执行受控本地写入。`/opc-thread` `/opc-seed` `/opc-backlog` 属于混合低摩擦 CLI：
> 查询只读，创建会写 `.opc/`。完整规则见 `AGENTS.md` §本地 runtime 白名单例外。
...
- **25 个斜杠命令**：16 派发器命令 + 6 本地 runtime 命令（/opc-health /opc-dashboard /opc-stats /opc-intel /opc-profile /opc-research）+ 3 混合低摩擦 CLI（/opc-thread /opc-seed /opc-backlog）
