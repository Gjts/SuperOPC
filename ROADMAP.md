...
- **已发布版本的实际内容** → 以 **[CHANGELOG.md](CHANGELOG.md)** 为准（语义化版本 + Keep a Changelog 格式）
- **未来版本的意图与愿景** → 以本文件为准
- **当前命令 / 技能 / 代理清单** → 以 `README.md` 架构树 + `agents/registry.json` + `skills/registry.json` 为准

本 ROADMAP 早期为 v1.1-v2.0 规划的内容中，有一部分已以更精简的形式在 v1.4 交付（花名与分类已重新划分）。**若本文件的路线与 CHANGELOG 记录冲突，以 CHANGELOG 为准**。

### 当前实际状态（截至 2026-04-22）

| 版本 | 主题 | 状态 | 参见 |
|------|------|------|------|
| v0.1.0 – v1.0.0 | 基础系统 → 正式发布 | ✅ 完成 | CHANGELOG `[0.1.0] - [1.0.0]` |
| v1.1.0 | 开发者画像 + 全局学习 | ✅ 完成 | CHANGELOG `[1.1.0]` |
| v1.2.0 | CLI 工具层 (`bin/opc-tools`) | ✅ 完成 | CHANGELOG `[1.2.0]` |
| v1.3.0 | Dispatcher 契约 + references/ 层 | ✅ 完成 | CHANGELOG `[1.3.0]` |
| v1.4.0 | Skill 精简到 17 个 + business-advisor | ✅ 完成 | CHANGELOG `[1.4.0]` |
| v1.4.1 | Skill Registry + Intent Router | ✅ 完成 | CHANGELOG `[1.4.1]` + ADR-0001/0002/0003 |
| v1.4.2 | 命令契约封堵 + cruise 真派发 | ✅ 完成 | CHANGELOG `[1.4.2]` + ADR-0004 |
